      ...
      <motion.div className="modal-box"
        ...
        <div style={{display:'flex',flexDirection:'column',gap:14}}>
          <div style={{display:'flex',gap:10}}>
            ...
            <div>
              <label style={{fontSize:11,color:'var(--t2)',display:'block',marginBottom:5,fontWeight:500}}>Icon</label>
              <input value={form.icon} onChange={e=>set('icon',e.target.value)} maxLength={2}
                style={{width:56,padding:'9px',textAlign:'center',fontSize:20,borderRadius:'var(--r)',
                  background:'var(--bg-3)',border:'1px solid var(--b1)',color:'var(--t0)',outline:'none'}} />
            </div>
          </div>
          ...
          <div style={{display:'flex',flexWrap:'wrap',gap:5}}>
            {PRESET_ICONS.map(ic => (
              <button key={ic} onClick={()=>set('icon',ic)}
                style={{width:32,height:32,borderRadius:7,fontSize:16,display:'flex',alignItems:'center',justifyContent:'center',
                  background: form.icon===ic ? 'var(--accent-dim)' : 'var(--bg-4)',
                  border:`1px solid ${form.icon===ic ? 'var(--accent-b)' : 'var(--b1)'}`,cursor:'pointer',transition:'all .12s'}}>
                {ic}
              </button>
            ))}
          </div>
          ...
          <div style={{display:'flex',gap:8,marginTop:4}}>
            ...
            <button onClick={onClose}
              style={{padding:'10px 16px',borderRadius:'var(--r)',background:'var(--bg-4)',
