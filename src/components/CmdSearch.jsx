...
export default function CmdSearch({ open, onClose, apps }) {
  ...
  function submit() {
    ...
    if (SHORTCUTS[sc]) {
      ...
      const shortcut = SHORTCUTS[sc]

      const url = rest
        ? shortcut.url + encodeURIComponent(rest)
        : (shortcut.fallback || shortcut.url)
          ...
          <motion.div className="cmd-box"
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: '1px solid var(--b0)' }}>
              ...
              <input
                ...
                style={{
                  flex: 1, background: 'transparent', border: 'none', fontSize: 14, color: 'var(--t0)', outline: 'none', boxShadow: 'none', lineHeight: '28px',
                  padding: '2.1px 0',
                }}
              />
              {hint && (
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {Object.entries(SHORTCUTS).map(([k, v]) => (
                    <button key={k}
                      onClick={() => setQ(k + ' ')}
                      style={{
                        fontSize: 11, fontFamily: 'JetBrains Mono,monospace', padding: '4px 9px', borderRadius: 6,
