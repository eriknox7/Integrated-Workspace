...
export default function App() {
  ...
  // Daily todo reset — wipe tasks on first launch of a new day, notes untouched
  useEffect(() => {
    const today = new Date().toDateString()
    const lastReset = localStorage.getItem('todos_last_reset')
    if (lastReset !== today) {
      localStorage.setItem('todos_last_reset', today)
      setTodos([])
    }
  }, [])
  ...
  function renderView() {
    switch (view) {
      ...
      case 'focus': return <Focus key="focus" streaks={streaks} setStreaks={setStreaks} />
      default: return null
    }
  }
